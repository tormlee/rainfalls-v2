...
get.classifier.lda <- function(data.train, data.test, ft.ind){
  get.info <- NULL
  if(ncol(data.train) != ncol(data.test)){
    ...
    # train model
    model = lda(data.train.x, data.train.y)
    train.data.ypred <- predict(model, data.train.x)$class
    test.data.ypred <- predict(model, data.test.x)$class
    # acc
