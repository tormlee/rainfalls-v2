...
bayes.rs.log <- "result/extreme/bayes.wet.dry.classification.ft.log"
# feature selection
  ...
  while(TRUE) {
    ...
    if(length(diff.ind) == 1){
      ...
      line.write <- sprintf("features = %s, train.acc = %f, test.acc = %f, train.rain.acc = %f, train.dry.acc = %f, test.rain.acc = %f, test.dry.acc = %f", 
                             bayes.classifier.max$features, bayes.classifier.max$train.acc, bayes.classifier.max$test.acc,
                             bayes.classifier.max$train.rain.acc, bayes.classifier.max$train.dry.acc, bayes.classifier.max$test.rain.acc, bayes.classifier.max$test.dry.acc)
      print(line.write)
