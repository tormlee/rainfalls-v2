...
library(caret)
data.train <- read.csv("data/tuning/extreme/data.train.extreme.v3.csv", header=TRUE)
data.test <- read.csv("data/tuning/extreme/data.test.extreme.v3.csv", header=TRUE)
svm.rs.log <- "result/extreme/svm.wet.dry.classification.ft.v3.log"
acc.filters <- c("mean", "harmonic-mean") # "train", "test", "and", "or", "mean", "harmonic-mean"
  ...
  while(TRUE) {
    ...
    if(length(diff.ind) == 1){
      ...
      line.write <- sprintf("features = %s, train.acc = %f, test.acc = %f, train.rain.acc = %f, train.dry.acc = %f, test.rain.acc = %f, test.dry.acc = %f", 
                             svm.classifier.max$features, svm.classifier.max$train.acc, svm.classifier.max$test.acc,
                             svm.classifier.max$train.rain.acc, svm.classifier.max$train.dry.acc, svm.classifier.max$test.rain.acc, svm.classifier.max$test.dry.acc)
      print(line.write)
